defmodule DpwmfJsonGenerator do
  ...
  def fetch_speaker(%{choice: true} = string),
    do: Map.put(string, :speaker, Constants.default_speaker())

  def fetch_speaker(%{speech: speech} = string) do
    case :binary.match(speech, ": ") do
      :nomatch ->
        Map.put(string, :speaker, Constants.default_speaker())
